...
// asserted on in a test, so this pins the pure decision instead: for a
// loopback bind there's only ever one line and it's used unchanged; for a
// bind beyond loopback, the first (scrim.local) line is used -- not the raw
// host:port URL a bind-all host like 0.0.0.0 would otherwise produce. The
// bind host is the whole condition; urlLines cannot see whether mDNS is
// actually advertising (see its doc).
func TestPrimaryURL(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "loopback host: single line used unchanged",
			host:     "127.0.0.1",
			rawURL:   "http://127.0.0.1:7777/?t=abc123",
			wantOpen: "http://127.0.0.1:7777/?t=abc123",
		},
		{
			name:     "LAN host: first line (scrim.local) used, not the raw URL",
			host:     "192.0.2.50",
			rawURL:   "http://192.0.2.50:7777/c/report/?t=abc123",
			wantOpen: "http://scrim.local:7777/c/report/?t=abc123",
		},
		{
			name:     "bind-all host: scrim.local used, not the unusable 0.0.0.0 URL",
			host:     "0.0.0.0",
